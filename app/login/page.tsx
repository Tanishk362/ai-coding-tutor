    ...
    <main className="min-h-screen flex items-center justify-center p-6 bg-black text-white">
      <div className="w-full max-w-md border border-white/10 rounded-xl p-6 bg-white/5">
        ...
        <form className="space-y-3" onSubmit={signInMagic}>
          <button disabled={loading || !email} className="w-full rounded bg-white/10 py-2 font-medium hover:bg-white/20">
            {loading ? "Sending…" : "Send magic link"}
          </button>
        </form>
        <p className="mt-4 text-sm text-slate-300">
          No account? <a href={`/signup?next=${encodeURIComponent(next)}`} className="text-blue-400 hover:underline">Create one</a>
        </p>
      </div>
    </main>
  );
